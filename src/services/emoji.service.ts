...
import { Emoji } from '../types/services.types';
...
export class EmojiService implements Emoji {
  private access_key: string = `${ACCESS_KEY_LABEL}=${ACCESS_KEY_VALUE}`;

  async loadSpacesEmojis(): Promise<void> {
    const emojis: string[] = await this.getAllEmojis();
    spacesEmojis.push(...emojis);
  }

  private async getAllEmojis(): Promise<string[]> {
